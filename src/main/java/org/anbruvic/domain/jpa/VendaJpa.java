...
public class VendaJpa implements Persistente {
    ...
    public void adicionarProduto(ProdutoJpa produto, Integer quantidade) {
        validarStatus();
        Optional<ProdutoQuantidadeJpa> op = produtos.stream().filter(filter -> filter.getProduto().getCodigo().equals(produto.getCodigo())).findAny();
        if (op.isPresent()) {
    ...
    public void removerProduto(ProdutoJpa produto, Integer quantidade) {
        ...
        if(op.isPresent()) {
            ProdutoQuantidadeJpa produtoQtd = op.get();
            if (produtoQtd.getQuatidade()>quantidade) {
    ...
    public Integer getQuantidadeTotalProdutos() {
        ...
        int result = produtos.stream().reduce(0, (partialCountResult, prod) -> partialCountResult + prod.getQuatidade(), Integer::sum);
        return result;
    }
